...
void ContactUserList::addContactUserList()
{
    ...
    //先创建自定义的item
    _add_friend_item = new ConUserItem(); //默认的item type是CONTACT_USER_ITEM
...
void ContactUserList::slot_item_clicked(QListWidgetItem *item)
{
    ...
    //判断不同的item类型，如果是group item不进行处理
    ...
    //申请好友item
    if(itemType == ListItemType::APPLY_FRIEND_ITEM){ //111行
        qDebug() << "apply friend item clicked ";
        emit sig_switch_apply_friend_page();
        return;
    }
    //联系人item
    if(itemType == ListItemType::CONTACT_USER_ITEM){  //ConUserItem类构造函数中默认设置itemtype为CONTACT_USEAR_ITEM
