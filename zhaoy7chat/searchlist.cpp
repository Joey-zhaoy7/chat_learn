...
void SearchList::slot_item_clicked(QListWidgetItem *item)
{
    ...
    /*点击添加好友item
     * 封装json数据，发送sig_send_data信号，通过TCP连接发送
     * TcpMgr调用write操作给服务器
     * TcpMgr监听socket，当有数据可读时，当读完时会调用handleMsg
     * handleMsg根据ReqId 调用相应的initHandler中的已经注册的函数
     * 注册的函数根据数据 对页面进行相应的逻辑操作
     */
