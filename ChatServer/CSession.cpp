...
void CSession::AsyncReadBody(int total_len)
{
	...
	asyncReadFull(total_len, [self, this, total_len](const boost::system::error_code& ec, std::size_t bytes_transfered) {
		try {
			...
			//更新session心跳时间
			//UpdateHeartbeat();
			//此处将消息投递到逻辑队列中 LogicSystem自己的工作线程会从队列中取数据
...
void CSession::AsyncReadHead(int total_len)
{
	...
	asyncReadFull(HEAD_TOTAL_LEN, [self, this](const boost::system::error_code& ec, std::size_t bytes_transfered) {
		//不会立即执行 读完4字节或者发生错误时执行下面回调函数
		// bytes_transfered是指handler传过来的参数 代表实际读取额字节数
		try {
			if (ec) {
				std::cout << "handle read failed, error is " << ec.what() << endl;
				Close();
...
//读取指定字节数 read_len已读取的字节数 total_len总共需要读取的字节数
void CSession::asyncReadLen(std::size_t read_len, std::size_t total_len,
	std::function<void(const boost::system::error_code&, std::size_t)> handler)
{
	auto self = shared_from_this();
	//bytesTransfered标识实际读取的字节数
	// total_len - read_len 告诉 async_read_some：
	// "你最多可以读取这么多字节，但不要超过这个数"
	_socket.async_read_some(boost::asio::buffer(_data + read_len, total_len - read_len),//指针和长度
		[read_len, total_len, handler, self](const boost::system::error_code& ec, std::size_t  bytesTransfered) {
			// 当操作系统完成读取后：
			// 数据已经被写入 _data + read_len 开始的内存中
			if (ec) {
				// 出现错误，调用回调函数
