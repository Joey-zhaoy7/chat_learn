...
void LogicSystem::GetUserByName(std::string uid_str, Json::Value& rtvalue)
{
	rtvalue["error"] = ErrorCodes::Success;

	std::string base_key = NAME_INFO + uid_str;
	...
	//查询数据库
	std::shared_ptr<UserInfo> user_info = nullptr;
	user_info = MysqlMgr::GetInstance()->GetUser(uid_str);
	if (user_info == nullptr) {
		rtvalue["error"] = ErrorCodes::UidInvalid;
		return;
	}
...
void LogicSystem::LoginHandler(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data) {
	...
	//为用户设置登录ip server的名字 为什么写入redis？ 
...
void LogicSystem::AddFriendApply(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
	...
	std::cout << "uid is " << uid << "applyname is " << applyname << "bakname is " << bakname << "touid is " << touid << std::endl;
	...
	//在同一个server 直接通知对方有申请消息
	if (to_ip_value == self_name) {
		auto session = UserMgr::GetInstance()->GetSession(touid); //同一个服务器 利用session
		if (session) {
			//在内存中则直接发送通知对方
			Json::Value  notify;
...
void LogicSystem::DealChatTextMsg(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
	...
	* 解析json
	* 查看对方服务器
	* 如果在同一个服务器直接利用session发送
	* 否则 通过grpc发送给对方服务器
	* 同样对方服务器的service再将信息转给客户端
	*/
	Json::Reader reader;
	Json::Value root;
	reader.parse(msg_data, root);
