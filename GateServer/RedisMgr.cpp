...
RedisConPool::~RedisConPool() {
    std::unique_lock<std::mutex> lock(mutex_);//为什么析构需要设置互斥锁 connections_为共享资源
...
redisContext* RedisConPool::getConnection() {
    ...
    cond_.wait(lock, [this] {
        if (b_stop_) {//b_stop_为private 需要捕获this? //编译器将lambda转化成一个对象，对象内部是一个独立的命名空间，无法访问其他类
            return true;
        }
        return !connections_.empty(); //不为空时返回true 执行回调，不阻塞进程
        });
...
void RedisConPool::returnConnection(redisContext* context) {
    std::lock_guard<std::mutex> lock(mutex_);//为什么归还连接也要设置锁?
    if (b_stop_) {
        return;//可能导致内存泄露，没有释放连接
    }
    connections_.push(context);
    //notify
    cond_.notify_one();//
...
void RedisConPool::close() {
    b_stop_ = true;
    cond_.notify_all();//唤醒所有阻塞等待的进程？这些进程会继续往下执行？
}
...
bool RedisMgr::Get(const std::string& key, std::string& value)
{
    ...
    if (con_pool_ == nullptr) {
        return false;
    }

    auto* reply = (redisReply*)redisCommand(connect, "GET %s", key.c_str());
    //reply = (redisReply*)redisCommand(this->_connect, "GET %s", key.c_str());
